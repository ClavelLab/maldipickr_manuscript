plot_dereplication_approaches <- function(all_results_clean){
  ...
  p<-all_results_clean |>
    ...
    dplyr::mutate(new_order = 1:dplyr::n()) |> dplyr::ungroup() |> 
    dplyr::group_by(procedure, membership) |>
    dplyr::add_count(name = "n_within_cluster") |>
    dplyr::mutate(
      n_label = dplyr::if_else(n_within_cluster > 1, paste0(n, "*"), as.character(n))
    ) |>
    dplyr::ungroup() |>
    dplyr::select(-n_within_cluster) |> 
    ggplot2::ggplot(aes(x = as.character(new_order), y = species_label, fill = as.character(n)))+
    ggplot2::geom_tile(color = "white", linewidth = 0.7)+
    ggplot2::geom_text(data = function(x) x[x$n %in% c(1,2,5),], aes(label=n_label), color = "white")+
    ggplot2::geom_text(data = function(x) x[!x$n %in% c(1,2,5),], aes(label=n_label), color = "black")+
    ggplot2::scale_y_discrete(labels = scales::label_parse())+
    ggplot2::scale_fill_manual(
      values = c("#000000FF", "#440154FF", "#2A788EFF", "#22A884FF", "#7AD151FF"),
      labels = c("1"="1 (absence of clustering)",
                 "8"="8 (expected clustering)"))+
    cowplot::theme_cowplot()+
    ggh4x::facet_manual(vars(procedure), design = design)+
    ggplot2::labs(x = "Clusters per mock species",
    ...
    ggplot2::theme(
      ...
      panel.grid.major.x = ggplot2::element_line(colour = "grey90", linewidth = 0.8)
    )
  return(p)
}

write_plot<-function(plot_results, path){
  ggplot2::ggsave(path, plot_results, height = 8, width = 8,device = grDevices::png, dpi = 300)
  return(path)
}